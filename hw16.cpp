...
struct Cmp_by_perimeter {
    bool operator()(const Shape* s1, const Shape* s2) const
    { return s1->calc_perimeter() > s2->calc_perimeter(); } // dereference pointer, compare perimeters
};
...
struct Cmp_by_area {
    bool operator()(const Shape* s1, const Shape* s2) const
    { return s1->calc_area() > s2->calc_area(); } // dereference pointer, compare areas
};
...
public:
	...
	Val low;
	Val high;
	bool operator()(const Val elem)const{
		return (elem < high && !(elem < low));
	}
...
int main()
{
	...
	cout << "*        CS1C 5/04/2020 1:30am       * " << endl;
	cout << "************************************** " << endl;
	cout << endl;
...
// Q#1
// Input Iterator must support sequential input operations and the type requirements are objects that can be compared with =

// Q#5
// copy_if must support Input Iterators, Output Iterators, and Predicates as parameters. In order for copy_if to work the Input iterator must be incrementable and dereferencable and it's value must be LessThan_comparable.
